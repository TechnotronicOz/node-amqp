...
Connection.prototype.addAllListeners = function() {
  ...
  // possible authentication error has occurred.  Although this may be a bug
  ...
  self.addListener('end', function (){
    if (!this.readyEmitted){
      ...
      // if we have already emitted the 'ready' event and the socket is
      // destroyed, we more then likely got disconnected from the server
      // so emit an error and attempt to reconnect.
      if (this.implOptions.reconnect && this.socket.destroyed) {
        this.emit('error', new Error('Connection ended: possibly due to disconnection from server.'));
      }
    }
  });
};
